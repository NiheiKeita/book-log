                ...
                <div className="relative flex items-end gap-4">
                    {row.map((userBook, index) => {
                        ...
                        return (
                            <div
                                key={userBook.id}
                                className={`group relative flex h-52 w-32 flex-col justify-between rounded-xl border border-amber-200/70 bg-gradient-to-b ${gradient} px-3 py-4 text-amber-900 shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-xl`}
                            >
                                <span className="absolute left-3 top-2 rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-semibold text-amber-700">
                                    #{number}
                                </span>
                                <span className="absolute right-3 top-0 h-9 w-9 -translate-y-1/2 rounded-full bg-white/60 text-center text-lg leading-9">
                                    🌷
                                </span>
                                <div className="space-y-2 pt-2">
                                    <div className="text-sm font-bold leading-snug text-amber-950">
                                        <span className="block max-h-10 overflow-hidden text-ellipsis whitespace-normal break-words">{book.title}</span>
                                    </div>
                                    <p className="truncate text-xs font-semibold text-amber-900/80">
                                        {book.author ?? '著者不明'}
                                    </p>
                                    <p className="text-[11px] text-amber-900/70">ISBN {book.isbn13}</p>
                                </div>
